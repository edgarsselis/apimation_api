...
def check_personal_info
  ...
  assert_equal(@test_user.email, response_hash['email'], 'Email in the response is not correct')
  assert_equal(@test_user.email, response_hash['login'], 'Login in the response is not correct')
  assert_equal(@test_user.user_id, response_hash['user_id'], 'User ID in the response is not correct')
end
...
def login_wrong_password
  ...
  assert_equal('Username or password is not correct', response_hash['error-msg'], 'Error message in response is not correct!')
...
def check_user_not_logged
  ...
  assert_equal('No session', response_hash['error-msg'], 'Error message in response is not correct!')
end